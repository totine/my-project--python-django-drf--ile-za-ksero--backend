...
def calculator(request):
    ...
    xero_cost = XeroSimpleCalc() if "xero_cost_id" not in request.session or not XeroCalc.get_xero_calc_by_id(request.session["xero_cost_id"]) \
...
def calculate_by_weight(request):
    ...
    bind_cost = Decimal(request.POST.get('bind_cost', 0))
    xero_cost = XeroByWeightCalc() if not "xero_cost_id" in request.session or not XeroCalc.get_xero_calc_by_id(request.session["xero_cost_id"]) or XeroCalc.get_xero_calc_by_id(request.session["xero_cost_id"]).cost_short_name != "weight"\
        else XeroCalc.get_xero_calc_by_id(request.session["xero_cost_id"])
    xero_cost.cost_per_page = cost_per_page
